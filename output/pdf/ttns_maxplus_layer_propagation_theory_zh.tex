...
F_{ka}(y_a-t_a-x)$ 关于 $x$ 可测，关于 $t$ 逐点连续于每个 $F_{ka}$ 的连续
点集；由于 $F_{ka}$ 单调，其不连续点至多可数，故该函数在
$\R\times\R^K$ 上联合可测。又因为所有 CDF 取值于 $[0,1]$，被积函数被
...
(I) 由引理 \ref{lem:nonexp}，$\mathcal A$ 是若干非扩张算子之复合，故
$\|\mathcal A(F_M-\widehat F_M)\|_\infty\le\|F_M-\widehat F_M\|_\infty$。而
$F_M-\widehat F_M$ 在每个求值点上正是推论 \ref{cor:local2joint} 中 $t$
固定时的被积量，故被 $\varepsilon^{\mathrm{loc}}(y)$ 控制。
