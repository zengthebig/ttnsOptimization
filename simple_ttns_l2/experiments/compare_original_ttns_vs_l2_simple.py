...
def _train_original_mle_ttns(
    ...
    t_prev = t_start
    print(
        f"\n=== [linear balanced target] [original_ttns_mle:{model_topology}] ===",
        flush=True,
    )
...
def _write_svg(rows: List[Dict], out_path: Path):
    ...
    lines.append(
        ...
        "Simple 6D Example: Original TTNS vs L2 TTNS"
        "</text>"
    )
    lines.append(
        "<text x='24' y='56' font-family='monospace' font-size='12' fill='#555'>"
        "Same balanced tree Gaussian target, same topology, different training objective."
        "</text>"
    )
...
def _write_report(
    ...
    lines.append("## 1. 实验设置")
    lines.append("")
    lines.append("- 目标分布: `6 维 balanced tree Gaussian`。")
...
def run():
    ...
    pairs: List[Tuple[int, int]] = [(0, 1), (0, 2), (2, 5)]
    grid_bins = 56
    n_slice_samples = 120000
    em_steps = 10

    print("running simple comparison: original TTNS(MLE) vs simple TTNS(L2)", flush=True)
    ...
    slice_x = sample_tree_gaussian(k_slice, n_slice_samples, parent_target, weights, noises)
    ...
    metrics_path = report_dir / "compare_original_ttns_vs_l2_simple_metrics.json"
    svg_path = report_dir / "compare_original_ttns_vs_l2_simple.svg"
    report_path = report_dir / "compare_original_ttns_vs_l2_simple_report_zh.md"
