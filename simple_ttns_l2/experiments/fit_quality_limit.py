...
def _fitlimit_config() -> ExperimentConfig:
...
def _fitlimit_lr_policy(cfg: ExperimentConfig) -> Dict:
    hold_steps = int(0.40 * cfg.train_steps)
    return {
        "mode": "delayed_cosine",
        "label": f"fitlimit_delayed_cosine_hold{hold_steps}_0.1x",
...
def _write_svg(rows: List[Dict], out_svg: Path):
    ...
    col_gap = 34
    row_gap = 96
    cols = ["target empirical", "model marginal", "abs diff"]
    ...
    lines.append(
        ...
        "Fit Limit Check (complex target, matched topology)"
        "</text>"
    )
    lines.append(
        "<text x='24' y='56' font-family='monospace' font-size='12' fill='#555'>"
        "Compare target vs matched-model 2D marginals. abs diff panel = |target - model|."
        "</text>"
    )
    ...
    for r, row in enumerate(rows):
        ...
        lines.append(
            ...
            f"IAE={row['iae']:.4f}, noise-floor IAE={row['noise_iae']:.4f}, "
            f"ratio={row['ratio_to_floor']:.2f}x, L2={row['l2']:.4f}</text>"
        )
        lines.append(
...
def _write_report(report_path: Path, cfg: ExperimentConfig, train_rows: List[Dict], slice_rows: List[Dict], svg_name: str):
    lines: List[str] = []
    lines.append("# Fit Limit Report")
    lines.append("")
    lines.append(
    ...
    lines.append("")
    lines.append("## Slice Fit Summary")
    lines.append("")
    lines.append("| target_topology | pair | IAE(model,target) | IAE(target,target2) | ratio_to_floor | L2 |")
    lines.append("|---|---:|---:|---:|---:|---:|")
    for row in slice_rows:
        pair = f"({row['dim_i']},{row['dim_j']})"
        lines.append(
            f"| {row['target_topology']} | {pair} | {row['iae']:.6f} | {row['noise_iae']:.6f} | "
            f"{row['ratio_to_floor']:.3f} | {row['l2']:.6f} |"
        )
...
def run_fit_limit():
    cfg = _fitlimit_config()
    lr_policy_template = _fitlimit_lr_policy(cfg)
    pairs: List[Tuple[int, int]] = [(0, 1), (0, 3), (2, 5)]
    grid_bins = 48
    n_slice_samples = 200000

    print("running fit limit check with config:", asdict(cfg), flush=True)
    ...
    for target_topology in ("balanced", "chain"):
        ...
        key = jax.random.PRNGKey(cfg.seed + 9000 + seed_cursor)
        seed_cursor += 100
        ...
        # Fit-limit mode: match topology.
        model_topology = target_topology
        ...
        for dim_i, dim_j in pairs:
            ...
            print(
                ...
                f"IAE={iae:.6f}, floor={noise_iae:.6f}, ratio={ratio:.3f}, L2={l2:.6f}",
                flush=True,
            )

    report_dir = REPO_ROOT / "simple_ttns_l2" / "reports"
    report_dir.mkdir(parents=True, exist_ok=True)
    out_svg = report_dir / "fit_limit_complex_matched.svg"
    out_md = report_dir / "fit_limit_complex_matched_report.md"
    out_json = report_dir / "fit_limit_complex_matched_metrics.json"
